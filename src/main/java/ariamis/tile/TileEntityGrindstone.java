...
import ariamis.net.IvNetworkHelperServer;
import ariamis.net.IvTileEntityHelper;
import ariamis.net.PartialUpdateHandler;
...
public class TileEntityGrindstone extends TileEntity implements PartialUpdateHandler
{
    ...
    public void updateEntity() {
        ...
        if (this.grindstoneRotationSpeed > 0) this.grindstoneRotationSpeed--;

        if (grindstoneHealth == 0) grindstoneRotationSpeed = 0;
        ...
        if (ticksTillSound > 0) ticksTillSound--;
    ...
    public boolean tryApplyingItem(ItemStack stack, EntityLivingBase entity) {
        if (stack != null && stack.getItem() == ItemRegistry.grindstoneStone) {
            if (grindstoneHealth == 0) {
                if (!worldObj.isRemote) {
                    this.grindstoneHealth = maxGrindstoneHealth;
                    stack.stackSize--;

                    IvNetworkHelperServer.sendTileEntityUpdatePacket(this, "grindstoneData", Ariamis.network);
                    markDirty();
                }
                return true;
            }
        ...
        if (stack != null) {
            ...
            if (grindstoneRotationSpeed > 60 && timeSharpening == 0) {
                ...
                if (item instanceof ItemSword) {
                    ...
                    if (toolMaterialName.equals("STONE") || toolMaterialName.equals("GOLD") || toolMaterialName.equals("IRON")) isRepairable = true;
                }
                ...
                if (isRepairable) {
                    ...
                    float itemSurvivalChance = new float[]{1f,0.9f,0.8f,0.5f,0.2f,0.3f,0.4f,0.5f,0.6f,0.7f,0.8f,0.8f,0.9f,0.9f} [currentItemRepairs % 14];
                    if (!getWorldObj().isRemote && getWorldObj().rand.nextFloat() > itemSurvivalChance) {
                        stack.stackSize--;
                        if (entity != null) entity.renderBrokenItemStack(stack);
                    } else {
                        currentItemRepairs++;
